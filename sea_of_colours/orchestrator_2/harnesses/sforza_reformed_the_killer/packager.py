...
def _harvester_demand(opt: Any) -> int:
    ...
    action = (getattr(opt, "payload", None) or {}).get("eye_action")
    if action:
        return int(action in {"claim", "direct"})
...
def _probe_demand(opt: Any) -> int:
    ...
    if kind in _PROBE_KINDS:
        return 1
    payload = getattr(opt, "payload", None) or {}
    if payload.get("eye_action") == "takeover":
        return 1
    if kind == "seam":
        n = 0
        for w in (payload.get("waves") or []):
...
def pack_recipe(
    ...
    *,
    ...
    for opt in ordered:
        ...
        payload = getattr(opt, "payload", None) or {}
        if payload.get("eye_action"):
            from . import strategy
            strategy.pack(pk, payload)
            continue
