# The Killer - Sforza Reformed

Participants: Alex Civita, Davinder Mundy, Tom Sprenger, Galiya Warrier,
Luis Arcia, Omri de Boers.

This agent uses
standard geometry metadata on PURE_PICK and FIRST_CLAIM so menu valuation and
inventory reconciliation price the actual PURE rather than zero. Correct pricing
does not force a PURE-only run ahead of a genuinely richer alternative.

After repair, empty-fleet recovery and a two-probe floor, this revision saves for
the second harvester before SNAP replenishment or speculative probe top-ups.
Once the second harvester exists, the one-SNAP budget resumes. BLUE gates remain.

Ordinary V12 probe-assisted exploration is available again. Reveal-first is now
scoped to unknown cells in rival redsign regions, not all unknown terrain. Both
the option menu and final move guard apply this restriction. H1 SNAP remains a
conditional tactic: rival probe-at-H1/drop-at-H2 can collide with our H2 grab.

In a five-game exploratory comparison against updated V12, this strategy won
three games with a mean score difference of +618.4. Games used seven days and
40x28 boards, with alternating seats (3-2 imbalance). No SNAP fired in that
sample: the result is not evidence of a weapon advantage or conclusive superiority.

Forked from updated V12. Existing model assignments, token limits and
THINK/PLAN topology remain unchanged.

`PURE_PICK*` drops on a live-known PURE and immediately lifts, including deliberate
sacrifice of our own probe under the landing. `FIRST_CLAIM*` instead SNAPs a contested
PURE at H1, drops at H2 with independent surviving coverage, and lifts at H3.
`CUT_THE_EYE*` SNAPs the single known supporting rival probe at H1 and places our
own probe at H2. `REVEAL_SIGN*` surveys without guessing the exact PURE coordinate.
Neither denial nor capture is guaranteed against unknown or replacement coverage.

Economy is fork-local: repairs, empty-fleet recovery, two-probe floor, saving for
the second harvester, then one SNAP, further fleet expansion and probe top-up.
No EMP/CHAFF buying. BLUE harvesting is
permitted only with no live-visible PURE/MASS, no SNAP and insufficient BLUE for
one round, using at most one deliberate BLUE outing. Final submitted moves are
checked even on mover/heuristic fallback paths; vetoes preserve hour slots with waits.
This stricter policy can leave units idle rather than make an ungrounded landing.

Custom code lives in `strategy.py`; agency, packager, prompt, orbit and final
harness hooks connect it. The scaffolded forge supplies schema, weapon-menu and
replay plumbing, but does not own the final coordinated tactics or purchase order.

Run candidate tests with `.venv/bin/python -m pytest
sea_of_colours/orchestrator_2/harnesses/sforza_reformed_the_killer/tests/test_strategy.py`.
Mocked submission proves the execution path, not natural model selection or advantage.

## Inherited V12 Documentation

The historical baseline description below does not describe the overrides above.

> ### This directory is yours — SFORZA_REFORMED_THE_KILLER
>
...
```44:75:sea_of_colours/orchestrator_2/harnesses/sforza_reformed_the_killer/orbit_policy.py
...
```43:43:sea_of_colours/orchestrator_2/harnesses/sforza_reformed_the_killer/_v7/chat_schema.py
...
Prefer changing `sforza_reformed_the_killer/` proper; touch `_v7/` only for shared
...
| --- | --- | --- |
| `SFORZA_REFORMED_THE_KILLER_SINGLE` | `0` | `1` skips the THINK/PLAN split — one LLM call, faster, dumber |
| `SFORZA_REFORMED_THE_KILLER_AUTOFILL` | `0` | `1` lets the packager top up a short plan |
