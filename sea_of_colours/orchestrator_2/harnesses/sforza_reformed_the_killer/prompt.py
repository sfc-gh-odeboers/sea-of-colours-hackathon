...
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer import (
    digest, doctrine, option_economics, out_of_grid, rules, world_view,
)
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer._v7.orbit_wishlist import (
    Wishlist,
)
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer._v7.probe_hints import (
    _redsign_centers,
    _enemy_probe_cells,
...
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer._v7.prompt import (
    format_state_block,
...
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer import weapon_forge
...
def build_prompt(
    *,
    ...
    parts.append("\nSFORZA_REFORMED_THE_KILLER POLICY: PURE_PICK takes only the known PURE then lifts; it may sacrifice our probe on landing. FIRST_CLAIM already includes H1 SNAP, H2 drop and H3 pickup: do not add another grab on that cell. CUT_THE_EYE already includes SNAP then a replacement probe: survey now and decide the exact harvest next planning turn. Do not invent PURE coordinates from a redsign centre. No EMP or CHAFF. Prefer direct capture if no rival contest is evident. SNAP denies only one square for one hour, not an entire enemy night.\n")
    parts.append("BLUE: one outing toward one SNAP only; stop at the missing ammunition budget.\n" if strategy.blue_needed(agent_view) else "BLUE: do not deliberately harvest BLUE tonight; visible PURE/MASS or existing ammunition takes priority.\n")
    return "".join(parts)
