"""sforza_reformed_the_killer — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``SFORZA_REFORMED_THE_KILLER_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.sforza_reformed_the_killer import prompt as prompt_mod
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "SFORZA_REFORMED_THE_KILLER"
THINKER_AGENT_LABEL = "SFORZA_REFORMED_THE_KILLER_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``SFORZA_REFORMED_THE_KILLER_SINGLE=1`` disables."""
    return os.environ.get("SFORZA_REFORMED_THE_KILLER_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``SFORZA_REFORMED_THE_KILLER_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("SFORZA_REFORMED_THE_KILLER_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
...
def run(
    *, store: Any, session_id: str, player: str, view: Mapping[str, Any],
    ...
    final_moves, strategy_log = strategy.guard(proposed[:_MAX_MOVES], agent_view)
    sanitizer_log.extend(strategy_log)
    if submit:
    ...
    payload = {
        ...
        "extras": {
            ...
            "sub_invocations": (
                [
                    {
                        "label": "SFORZA_REFORMED_THE_KILLER_THINK",
                        "kind": "think",
                        "api": "chat",
